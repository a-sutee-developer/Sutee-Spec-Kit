...
generate_commands() {
  ...
  mkdir -p "$output_dir"
  for template in templates/commands/{design,implement,requirement,task}.md; do
    [[ -f "$template" ]] || continue
...
ls -1 "$GENRELEASES_DIR"/sutee-spec-kit-template-*-"${NEW_VERSION}".zip
